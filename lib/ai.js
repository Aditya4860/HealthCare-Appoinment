...
exports.getPreVisitSummary = getPreVisitSummary;
exports.getPostVisitSummary = getPostVisitSummary;
...
Symptoms: ${symptoms}`,
        ...
        if (Array.isArray(questions)) {
            questions = questions
                ...
                .map((q) => q.replace(/^\d+[\.\)]\s*/, '').trim()); // Strip any preceding numbers like "1." if LLM added them
...
async function getPostVisitSummary(notes, prescription) {
    try {
        const response = await ai.models.generateContent({
            model: "gemini-3.6-flash",
                    ...
                    properties: {
                        ...
                        medicationSchedule: {
                            ...
                            items: {
                                type: genai_1.Type.OBJECT,
                                properties: {
                                    medicine: { type: genai_1.Type.STRING },
                                    dose: { type: genai_1.Type.STRING },
